...
on_test(function(package)
	assert(
		package:check_cxxsnippets(
			[[ void test() { wgpu::Instance instance; } ]],
			{ includes = "wgpu-native/webgpu.hpp", configs = { languages = "c++17" } }
		)
	)
end)
package_end()
...
on_test(function(package)
	assert(
		package:has_cfuncs(
			"SDL_GetWGPUSurface",
			{ includes = "sdl2webgpu.h", configs = { defines = "SDL_MAIN_HANDLED" } }
		)
	)
end)
package_end()
...
target("wgpu-test")
set_kind("binary")
add_packages("wgpu-native", "libsdl2", "sdl2webgpu", "wgpu-native-cpp", "glm", "tinyobjloader", "stb", "imgui")
...
add_headerfiles("inc/imgui/backends/*.h")
