...
// MARK: Size
TEST_CASE("Vector2 is always 8 bytes in size", "[vector-size]") {
...
TEST_CASE("Comparison operations of a vector", "[vector-comparison]") {
    Vector2 vec1(-3.0f, 1.0f);
    Vector2 vec2( 4.0f, 4.0f);

    INFO("vec1 = [-3.0,  1.0]");
    INFO("vec2 = [ 4.0,  4.0]");

    SECTION("Check equality of two vectors", "[comparison-equality]") {
        REQUIRE(vec1 == Vector2(-3.0f, 1.0));
    }
...
TEST_CASE("Arithmetic operations of a vector", "[vector-arithmetic]") {
    float scalar = 2.0f;
    Vector2 vec1( 2.0f, -7.0f);
    Vector2 vec2(-5.0f,  3.0f);
...
TEST_CASE("Basic operations of a vector", "[vector-basic]") {
    Vector2 vec1(2.0f, 6.0f);
    Vector2 vec2(-3.0, 4.0f);
    
    INFO("vec1 = [ 2.0, 6.0]");
    INFO("vec2 = [-3.0, 4.0]");

    SECTION("Normalization operation on a vector", "[vector-normalization]") {
        Vector2 result = Vector2::Normalize(vec1);
    ...
    SECTION("Magnitude operation on a vector", "[vector-magnitude]") {
        REQUIRE(std::round(Vector2::Magnitude(vec1) * 1000.0f) / 1000.0f == 6.325f);
    }

    SECTION("Dot operation on two vectors", "[vector-dot]") {
        REQUIRE(Vector2::Dot(vec1, vec2) == 18.0f);
    }
}